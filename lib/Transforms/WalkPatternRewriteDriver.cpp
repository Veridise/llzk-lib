...
//
// This file has been ported from a more recent version of LLVM with no changes.
//
//===----------------------------------------------------------------------===//
...
namespace {
struct WalkAndApplyPatternsAction final : tracing::ActionImpl<WalkAndApplyPatternsAction> {
...
struct ErasedOpsListener final : RewriterBase::ForwardingListener {
  ...
  void checkErasure(Operation *op) const {
    ...
    if (ancestorOp != visitedOp) {
      llvm::report_fatal_error("unsupported erasure in WalkPatternRewriter; "
...
void walkAndApplyPatterns(
    Operation *op, const FrozenRewritePatternSet &patterns, RewriterBase::Listener *listener
) {
#if MLIR_ENABLE_EXPENSIVE_PATTERN_API_CHECKS
  if (failed(verify(op))) {
  ...
  struct PatternRewriterImpl : public PatternRewriter {
    PatternRewriterImpl(MLIRContext *ctx) : PatternRewriter(ctx) {}
  };

  MLIRContext *ctx = op->getContext();
  PatternRewriterImpl rewriter(ctx);
#if MLIR_ENABLE_EXPENSIVE_PATTERN_API_CHECKS
...
#if MLIR_ENABLE_EXPENSIVE_PATTERN_API_CHECKS
  if (failed(verify(op))) {
    llvm::report_fatal_error("walk pattern rewriter result IR failed to verify");
  }
#endif // MLIR_ENABLE_EXPENSIVE_PATTERN_API_CHECKS
}
